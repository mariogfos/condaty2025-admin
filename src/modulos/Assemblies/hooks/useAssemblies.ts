...
import { Assembly, AssemblyAttendance, AssemblyStats } from "../types/assemblies.types";
...
interface UseAssembliesReturn {
  ...
  updateAssemblyStatus: (id: string | number, status: string) => Promise<boolean>;
  execute: any;
  reLoad: any;
}
...
export const useAssemblies = (): UseAssembliesReturn => {
  ...
  const fetchAssemblies = useCallback(async (params: any = { fullType: "L" }) => {
    setLoading(true);
    setError(null);
    try {
      const { data: response } = await execute(modulePath, "GET", params, true);
      if (response?.success) {
  ...
  const fetchAssemblyDetail = useCallback(async (id: string | number): Promise<Assembly | null> => {
    setLoading(true);
    setError(null);
    try {
      const { data: response } = await execute(modulePath, "GET", { fullType: "DET", searchBy: id }, false, true);
      if (response?.success) {
  ...
  const fetchAssemblyStats = useCallback(async (id: string | number): Promise<AssemblyStats | null> => {
    try {
      const { data: response } = await execute(`${modulePath}/${id}/stats`, "GET", {}, false, true);
      if (response?.success) {
        return response.data || null;
      }
      return null;
  ...
  const updateAssemblyStatus = useCallback(async (id: string | number, status: string): Promise<boolean> => {
    try {
      const { data: response } = await execute(`${modulePath}/${id}/status`, "PUT", { status }, false, true);
      if (response?.success) {
