...
const getTodayIsoDate = () => fechaLocalIso();

const getYearStartIsoDate = () => `${new Date().getFullYear()}-01-01`;

const getMonthStartIsoDate = () => `${getTodayIsoDate().slice(0, 7)}-01`;
...
const getReportDownloadHeaders = (): Record<string, string> => {
  ...
  // 🔴 `test` armaba la clave a mano con `"/adm-iam"` de respaldo; la que
  // escribe el login usa `"/v3/adm-iam"`. Sin la variable en el build, las
  // descargas salían sin token.
  try {
    const storedSession = JSON.parse(localStorage.getItem(CLAVE_DEL_TOKEN) || "null");
