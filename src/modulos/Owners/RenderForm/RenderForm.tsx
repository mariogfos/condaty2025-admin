...
const RenderForm = ({
  ...
  const selectedUnitDisplay = useMemo(() => {
    ...
    const unit = unitsList.find(
      (d: any) => String(d.dpto_id) === String(first.dpto_id),
    );
    return unit ? `Unidad ${unit.nro}` : `Unidad ${first.dpto_id}`;
  }, [
    formState.dptos,
    formState.type_owner,
    extraData?.dptosForH,
    extraData?.dptosForT,
  ]);
        ...
        {(formState.dptos || []).length > 0 && (
          <div className={styles.unitCardsWrapper}>
            {(formState.dptos || []).map((d, idx) => {
              ...
              const nro = d.dpto_nro
                ? getUnitNro(unitsList, d.dpto_nro)
                : getUnitNro(unitsList, d.dpto_id);
              return (
                <div
                  key={String(d.dpto_id) + "_" + idx}
        ...
        onSave={(data) => {
          ...
          const unitsList =
            formState.type_owner === "Propietario"
              ...
              : extraData?.dptosForT || [];
          const unit = unitsList.find(
            (u: any) => String(u.id) === String(normalizedId),
          );
          ...
          setFormState((prev) => {
            ...
            if (prev.type_owner === "Residente") {
              return {
                ...prev,
                dptos: [{ dpto_id: normalizedId, nro }],
              };
            }
            ...
            if (existing) {
              newDptos = newDptos.map((d) =>
                String(d.dpto_id) === String(normalizedId) ? { ...d, nro } : d,
              );
            } else {
              newDptos.push({
