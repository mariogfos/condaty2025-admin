...
export const SurveyDashboard: React.FC<SurveyDashboardProps> = ({
  ...
  const handleRunAI = async () => {
    ...
    try {
      const res = await execute(
        ...
        false,
        true,
      ); // notWaiting: true to avoid global loading

      if (res.data?.success) {
        showToast(
          "Análisis de IA iniciado. Los resultados aparecerán progresivamente.",
          "success",
        );
      }
    } catch (error) {
  ...
  const renderQuestionStats = (q: any) => {
          ...
          <div className={styles.keywordSection}>
            ...
            <div className={styles.keywordCloud}>
              {Object.entries(q.top_keywords || {}).length > 0 ? (
                Object.entries(q.top_keywords || {}).map(
                  ([word, freq]: [string, any]) => (
    ...
    const complexCategories = q.options.map((o: any, i: number) => {
      ...
      if (isScale) {
        ...
        if (i === q.options.length - 1 && q.label_last)
          label = `${o.text} (${q.label_last})`;
      }
      return label;
    });
    ...
    const series = [
      {
        name: "Votos",
        data: q.options.map((o: any) => o.votes ?? o.count ?? 0),
      },
    ];
          ...
          <div className={styles.statsRow}>
            <div className={styles.statBadge}>
              ...
              <span className={styles.statValue}>
                {Number(q.advanced_stats.average || 0).toFixed(1)}
              </span>
            </div>
            <div className={styles.statBadge}>
              <span className={styles.statLabel}>Mediana:</span>
              <span className={styles.statValue}>
                {q.advanced_stats.median || 0}
              </span>
            </div>
            <div className={styles.statBadge}>
              <span className={styles.statLabel}>I. Consenso:</span>
              <span className={styles.statValue}>
                {q.advanced_stats.std_dev !== undefined
                  ? (1 - q.advanced_stats.std_dev / 4.5).toFixed(2)
                  : "1.00"}
              </span>
            </div>
          </div>
  ...
  const renderAIQuestionAnalysis = (questionId: number) => {
    const report = aiReports.find(
      (r) => Number(r.squestion_id) === Number(questionId),
    );
        ...
        {report.metadata?.keywords?.length > 0 && (
          <div className={styles.aiKeywords}>
            {report.metadata.keywords.map((k: string) => (
              <span key={k} className={styles.aiTag}>
                #{k}
              </span>
            ))}
          </div>
    ...
    <div className={styles.dashboardContainer}>
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>
          Análisis detallado de participación y resultados
        </p>
      </header>
      ...
      <section className={styles.aiSection}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: "0.75rem", opacity: 0.7 }}>
                {aiAnalysis
                  ...
                  : "Aún no se ha generado un análisis global"}
              </p>
            </div>
          </div>
        ...
        {aiAnalysis ? (
          <div className={styles.aiContent}>{aiAnalysis.content}</div>
        ) : (
          !aiLoading && (
            <p className={styles.questionMeta} style={{ fontStyle: "italic" }}>
              Haz clic en el botón superior para generar un resumen ejecutivo de
              toda la encuesta basado en los datos actuales.
            </p>
          )
        )}
      </section>
      ...
      <div className={styles.questionsGrid}>
        {questions.map((q: any) => {
          const totalVotes =
            q.total_responses ??
            q.options.reduce(
              (acc: number, o: any) => acc + (o.votes ?? o.count ?? 0),
              0,
            );
          return (
            <div key={q.id} className={styles.questionCard}>
              <div className={styles.questionHeader}>
                ...
                <p className={styles.questionMeta}>
                  {q.type_label} • {totalVotes}{" "}
                  {totalVotes === 1 ? "respuesta" : "respuestas"}
                </p>
              </div>
