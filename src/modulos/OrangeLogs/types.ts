 ...
 * `GET /api/v3/orange-webhook-logs`.
 *
...
export interface OrangeLogRow {
  ...
  /**
   * 🔴 Es un NÚMERO (`DebtStatus`, 1..10), no la letra que mandaba la rama de
   * donde salió esta pantalla. No se traduce acá: el API manda la etiqueta al
   * lado, y tener un segundo diccionario de estados en el front es garantizar
   * que algún día digan cosas distintas.
   */
  debt_status_after: number | null;
  /** «Por cobrar», «Cobrada», … Lo arma `DebtStatus::label()` en el servidor. */
  debt_status_after_label: string | null;
...
export const ACTION_COLOR: Record<string, { color: string; bg: string }> = {
  ...
  voucher_adjuntado:   { color: '#22683F', bg: '#DCF5E5' },
  voucher_fallido:     { color: '#8A5A00', bg: '#FDF0D5' },
};
