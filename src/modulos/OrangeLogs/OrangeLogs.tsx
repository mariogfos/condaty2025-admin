...
const OrangeLogs = () => {
  ...
  const cargar = useCallback(
    async (f: OrangeLogFilters) => {
      // 🔴 La ruta lleva `v3`. En la rama de donde salió esta pantalla vivía en
      // `/api/orange-webhook-logs`; acá eso es un 404 y la pantalla mostraría
      // «No se pudo leer el log de Orange» sin decir que la ruta no existe.
      const respuesta = await pedirLog('/v3/orange-webhook-logs', 'GET', armarParams(f));
  ...
  useEffect(() => {
    ...
    void (async () => {
      const respuesta = await pedirAcciones('/v3/orange-webhook-logs/actions', 'GET', {});
