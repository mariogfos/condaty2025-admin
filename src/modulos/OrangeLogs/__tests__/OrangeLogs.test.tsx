...
const LINEA_CUOTA = {
  ...
  periodo: "2026-03",
  amount: 833.85,
  // 🔴 NÚMERO, no la letra: acá `debt_dptos.status` es `DebtStatus` (5 =
  // PAID). Y la etiqueta la manda el servidor al lado, ya traducida.
  debt_status_after: 5,
  debt_status_after_label: "Cobrada",
...
const llamadasAlLog = () =>
  executeMock.mock.calls.filter((c) => String(c[0]) === "/v3/orange-webhook-logs");
...
describe("OrangeLogs — el log del webhook en el backoffice", () => {
  ...
  it("muestra la unidad, el período y cómo quedó la deuda de cada línea", async () => {
    ...
    // El estado se muestra con la etiqueta que manda el API. Un "5" pelado no
    // le dice nada a administración, y traducirlo acá sería un segundo
    // diccionario de estados que algún día va a discrepar del del servidor.
