...
export function FormattedDateWithSlash(dateString: string) {
  if (!dateString || dateString == "") return "";
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateString)) {
    console.warn("La fecha debe estar en formato YYYY-MM-DD");
  }
  if (!dateString || typeof dateString !== "string") {
    return "Fecha no disponible"; // Retorna un valor predeterminado si la fecha no es válida
  }
...
export function calculateTimeDifference(
  ...
  if (diffInMs < 0) {
    throw new Error(
      "La fecha de creación no puede ser mayor que la fecha actual"
    );
  }
...
export function getTimePMAM(
  ...
  if (utc) {
    const fechaLocal: Date | null = convertirFechaUTCaLocal(dateStr);
    if (fechaLocal) {
      date = fechaLocal;
    }
  }
